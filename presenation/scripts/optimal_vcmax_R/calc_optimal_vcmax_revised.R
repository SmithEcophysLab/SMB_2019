...
calc_optimal_vcmax <- function(tg_c = 25, z = 0, vpdo = 1, cao = 400, paro = 800, q025 = 0.162, theta = 0.85){
  ...
  km <- calc_km_pa(tg_c, z)                # Eq. 3
	...
	# calculate q0
	q0_star = 0.257
	q0_ns = q0_star * phi_ftemp(tg_c)
	q0_cb = -0.0805 + (0.022 * tg_c) - (0.00034 * tg_c * tg_c)
	
	# calculate vcmax and jmax	
	vcmax_star <- ((q0_star * par * m) / mc) * (omega_star / (8 * theta))	          # Eq. 19
	#vcmax_prime <- ((q0_ns * par * m) / mc) * (omega_star / (8 * theta))	        # Eq. 20 >> revised
	vcmax_prime = ((q0_cb * par * m) / mc) * (omega_star / (8 * theta))
	vcmax_prime_cb = ((q0_cb * par * m) / mc) * (omega_star / (8 * theta))	
	jvrat <- ((8 * theta * mc * omega) / (m * omega_star))     # Eq. 15 / Eq. 19
	jmax_star = jvrat * vcmax_star
	jmax_prime = jvrat * vcmax_prime
	jmax_prime_cb = jvrat * vcmax_prime_cb
	...
	# output
	results <- as.data.frame(cbind(tg_c, z, vpdo, cao, paro, q0_star, q0_ns, q0_cb, theta, c, to, par, patm, ca, vpd, chi, ci, km, 
	                               gammastar, omega, m, mc, omega_star, vcmax_star, vcmax_prime, vcmax_prime_cb,
	                               jvrat, jmax_star, jmax_prime, jmax_prime_cb))
	
	colnames(results) <- c('tg_c', 'z', 'vpdo', 'cao', 'paro', 'q0_star', 'q0_ns', 'q0_cb', 'theta', 'c', 'to', 
	                       'par', 'patm', 'ca', 'vpd', 'chi', 'ci', 'km', 
	                       'gammastar', 'omega', 'm', 'mc', 'omega_star', 'vcmax_star', 'vcmax_prime', 'vcmax_prime_cb',
	                       'jvrat', 'jmax_star', 'jmax_prime', 'jmax_prime_cb')
